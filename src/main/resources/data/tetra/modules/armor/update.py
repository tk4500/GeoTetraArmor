...
def deep_merge(origem, atualizacao):
    ...
    for chave, valor in atualizacao.items():
        if isinstance(valor, dict) and chave in origem and isinstance(origem[chave], dict):
        ...
        if nome_alvo in arquivos:
            ...
            try:
                ...
                if "variants" in conteudo and len(conteudo["variants"]) > 0:
                    ...
                    conteudo["variants"][0] = deep_merge(variante_original, novos_dados_variante)
...
nome_do_json = 'heavy.json' # Ou o nome do arquivo que você quer filtrar
...
parametros_para_injetar = {
    ...
    "variants": [
        {
            "materials": [
                "tetra:wood/", "tetra:stone/", "tetra:gem/", "tetra:metal/",
                "tetra:bone/", "tetra:fabric/", "tetra:fibre/", "tetra:skin/", "tetra:scale/"
            ],
            "durability": 4, # Note que no seu exemplo você enviou dois valores de durability, o Python pegará o último
